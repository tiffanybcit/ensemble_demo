...
const mongoURI = "mongodb+srv://admin:qwerty123456@nemesistest.k4jez.mongodb.net/test";
...
app.use(function (req, res, next) {
    ...
    res.setHeader('Access-Control-Allow-Origin', 'https://ensemble-tiffany-demo.herokuapp.com/');
...
app.get("/readSalesData", (req, res) => {
    ...
    MongoClient.connect(mongoURI, {
        ...
        if (err) throw err;
        var dbo = db.db("nemesis_project");
        dbo.collection("sales_demo").find({}).toArray(function (err, result) {
...
app.get("/readLaborAndSales", (req, res) => {
        ...
        MongoClient.connect(mongoURI, {
            ...
            if (err) throw err;
            var dbo = db.db("nemesis_project");
            dbo.collection("labor_demo").find({}).toArray(function (err, result) {
                result1 = result;
                if (err) throw err;
                db.close();
            });
...
app.post("/writeLaborCost", (req, res) => {
    ...
    MongoClient.connect(mongoURI, {
        ...
        for (let item of req.body.rowobj) {
            dbo.collection("labor_sales").insertOne({
